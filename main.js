// 일정 목록을 저장하는 배열 (변경될 때마다 Firebase Realtime Database에도 저장되어 가족 구성원 간 공유됨)
...
// Firebase Realtime Database의 "todos" 경로 참조 (firebase-config.js에서 초기화된 firebase 앱 사용)
const todosRef = firebase.database().ref("todos");

// 현재 todos를 Firebase에 저장해 다른 가족 구성원과 공유하는 함수
function saveTodos() {
  todosRef.set(todos).catch((error) => {
    console.error("일정을 저장하지 못했습니다:", error);
    alert("일정을 저장하지 못했습니다. 인터넷 연결을 확인해주세요.");
  });
}

// Firebase에서 최신 todos를 한 번 불러오는 함수 (새로고침할 때마다 호출되어 다른 구성원의 변경사항과 동기화됨)
function loadTodos(callback) {
  todosRef
    ...
    .catch((error) => {
      ...
      alert("일정을 불러오지 못했습니다. 인터넷 연결을 확인해주세요.");
      todos = [];
      callback();
    });
}
...
// 새로고침 버튼을 눌렀을 때: Firebase에서 최신 일정을 다시 불러와 목록/달력에 반영
